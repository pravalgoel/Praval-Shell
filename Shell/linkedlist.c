...
int delete_from_list(int pid)
{
	...
	free(del); //jo unused pointer hai unhe dlt krne k liye 
    del = NULL;
    return 0;
}
...
void kill_list()
{
	...
	while(ptr != NULL)
	{
		if(kill(ptr->pid, SIGTERM) >= 0)	delete_from_list(ptr->pid);
		else{
			perror("Attempt to kill the entire list");
			printf("process counld not be killed %d\n",ptr->pid);
		}
		ptr = ptr->next;
	}
